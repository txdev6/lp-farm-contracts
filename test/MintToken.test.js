...
describe('MintToken', function () {
    ...
    beforeEach(async () => {
        ...
        this.piggyToken = await MintToken.new();
    ...
    it('should have correct name and symbol and decimal', async () => {
     ...
     //    assert.equal(decimals.valueOf(), '18');
    	// assert.equal()
        expect(await this.piggyToken.name()).to.equal('Mint Coin');
        expect(await this.piggyToken.symbol()).to.equal('MC');
        expect(await this.piggyToken.decimals()).to.be.bignumber.equal(this.value);
    });

    it('should only allow owner to mint token', async () => {

        await this.piggyToken.grantRole(this.role, alice);
